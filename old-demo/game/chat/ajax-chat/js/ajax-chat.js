...
if (getCookie('chat_colr'))
	...
	chat_colr = '#'+(Math.floor(6*Math.random())*32+48).toString(16)
...
function chat_date(offset)
{
	...
	if ((d2.getDate() == d1.getDate()) && (d2.getMonth() == d1.getMonth()))
		...
		return chat_date_aux(d2.getDate())+' '+mm[d2.getMonth()]+', '+chat_date_aux(d2.getHours())+':'+chat_date_aux(d2.getMinutes());
}
...
function chat_login(asuser)
{
	...
	if (chat_focu)
		if (document.getElementById('glogin').style.display == 'block')
...
{
	if (chat_msgs[user1] == undefined)
		chat_msgs[user1] = new Array();

	if (chat_msgs[user2] == undefined)
		chat_msgs[user2] = new Array();

	if (chat_msgs[user1][user2] == undefined)
		chat_msgs[user1][user2] = '';

	if (chat_msgs[user2][user1] == undefined)
		chat_msgs[user2][user1] = '';

	if (chat_wait[user1] == undefined)
		chat_wait[user1] = new Array();

	if (chat_wait[user2] == undefined)
		chat_wait[user2] = new Array();

	if (chat_wait[user1][user2] == undefined)
		chat_wait[user1][user2] = false;

	if (chat_wait[user2][user1] == undefined)
		chat_wait[user2][user1] = false;
}
...
function chat_priv_switch(user, focus)
{
	...
	if (user == '.')
		...
		document.getElementById('header_messages').innerHTML = user;
...
{
	if (!chat_user || !chat_pass)
	{
		chat_login(false);
		return;
	}

	if (!document.getElementById('send').value || chat_XMLHttp_add.readyState % 4)
		return;
...
function chat_msgs_get()
{
	...
	if (chat_XMLHttp_get.readyState % 4)
		return;
	...
	chat_XMLHttp_get.onreadystatechange = function()
	                                      {
		                                      if(chat_XMLHttp_get.readyState == 4 && chat_XMLHttp_get.status == 200)
		                                      {
			                                      ...
			                                      for (var i = 1; i < data.length-1; i++)
			                                      {
				                                      ...
				                                      {
					                                      if (chat_smsg)
						                                      if (data[i+2] == chat_room)
							                                      chat_msgs['.'] += '<b>System:</b> user <b>'+chat_msgs_usr(data[i+3], 'white')+'</b> enters the room<br />';

					                                      chat_usrs[data[i+3]] = data[i+2];
				                                      ...
				                                      if (data[i+1] == '-')
				                                      {
					                                      if (chat_smsg)
						                                      if (data[i+2] == chat_room)
							                                      chat_msgs['.'] += '<b>System:</b> user <b>'+chat_msgs_usr(data[i+3], 'white')+'</b> leaves the room<br />';
				                                      ...
				                                      if (data[i+1] == 'm')
				                                      {
					                                      chat_usrs[data[i+3]] = chat_room;
					                                      data[i+5] = data[i+5].replace(/%%(\w+)%%/g, '<img src="'+chat_path+'smileys/$1.gif" alt="" />');

					                                      if (data[i+4] == '.')
						                                      chat_msgs['.']                  += '<span style="color: '+data[i+2]+'"><b>['+chat_date(-data[i+6])+'] '+chat_msgs_usr(data[i+3], data[i+2])+'</b>:  '+data[i+5]+'</span><br />';
					                                      else
					                                      {
						                                      chat_priv_prepair(data[i+3], data[i+4]);
						                                      chat_msgs[data[i+3]][data[i+4]] += '<span style="color: '+data[i+2]+'"><b>['+chat_date(-data[i+6])+'] '+chat_msgs_usr(data[i+3], data[i+2])+'</b>:  '+data[i+5]+'</span><br />';
						                                      chat_msgs[data[i+4]][data[i+3]] += '<span style="color: '+data[i+2]+'"><b>['+chat_date(-data[i+6])+'] '+chat_msgs_usr(data[i+3], data[i+2])+'</b>:  '+data[i+5]+'</span><br />';
                                      ...
                                      function chat_msgs_log(asuser)
                                      {
	                                      ...
	                                      chat_XMLHttp_log.onreadystatechange = function()
                                                                            ...
                                                                            function chat_msgs_usr(user, color)
                                                                            {
	                                                                            return '<a style="color: '+color+'" href="javascript:chat_priv_switch(\''+user+'\', true);">'+user+'</a>';
                                                                            }
                                                                            ...
                                                                            function chat_out_usrs()
                                                                            {
	                                                                            ...
	                                                                            for (var i in chat_usrs)
		                                                                            if (i != chat_user && chat_usrs[i] && chat_wait[chat_user] != undefined && chat_wait[chat_user][i] != undefined &&  chat_wait[chat_user][i])
			                                                                            users = '<img src="'+chat_path+'style/user.png" alt="" />'+chat_msgs_usr(i,         'white')+'<br />'+users;
	                                                                            ...
	                                                                            for (var i in chat_usrs)
		                                                                            if (i != chat_user && chat_usrs[i] && chat_usrs[i] == chat_room)
			                                                                            users = '<img src="'+chat_path+'style/user.png" alt="" />'+chat_msgs_usr(i,         'white')+' <br />'+users;

	                                                                            if (chat_user)
		                                                                            users = '<img src="'+chat_path+'style/user.png" alt="" />'+chat_msgs_usr(chat_user, 'white')+' <br />'+users;
