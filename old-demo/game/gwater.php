...
?>
<?php 
$stmt = $db->prepare('UPDATE inventory SET waterunits = maxwater where charname = :charname and maxwater>0') ;
					$stmt->execute(array(':charname' => $charname));
$stmt = $db->prepare("SELECT cond FROM charstats WHERE charname=:charname");
				$stmt->execute(array(':charname' => $charname));
...
<table class=\"page\"><tr><td class=\"center\"><a class=\"small\" href=\"maingraphics.php?";
