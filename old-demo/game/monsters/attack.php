...
$report = date("h:i:sa").': You ATTACK!<br />';
$counter = $enemyname." COUNTER-ATTACKS!<br />";
...
//**********miss**********
  ...
  //***************enemy agility check*********
  $miss = $miss + $eagility + $eluck;
  if ($miss>$rand) {
  $report=$report."<span class=\"green\">You <b>Miss!</b></span><br />";
  ...
  $rand = $rand + $accuracy;
  if ($evade>$rand) {
  $report=$report."<span class=\"green\">The enemy <b>evades!</b></span><br />";
  include ('php/cendturn.php');  
  }
  else {
  //***enemy block chance***
  $block=$estrength+$espeed-$speed;
  $rand=mt_rand(1,100);
  if ($block>$rand) {
  $report=$report."<span class=\"green\">Your enemy <b>Blocks!</b></span><br />";
//***FUTURE  include attackbonus
  include ('php/cendturn.php');
  }
  else {
  //SKIPPING POWERS FOR NOW **************************************find them at monster/php/powers.php
  include ('php/typedamage.php');
//****FUTURE includes/attackbonus
